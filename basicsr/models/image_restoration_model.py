...
from basicsr.utils import get_root_logger, imwrite, splitimage, mergeimage
...
class ImageRestorationModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_pix:
            ...
            if self.opt['network_g'].get('type') == 'SRN':
                l_pix += self.cri_pix(preds, self.gt) #  SRN
            else:
                for pred in preds:  #  add the loss of all preds by model!
        ...
        l_total = l_total + 0 * sum(p.sum() for p in self.net_g.parameters())  # L1 regulation
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        dataset_name = self.opt['val'].get('name')
