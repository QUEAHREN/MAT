...
class TestImageEventRestorationModel(BaseModel):
    ...
    def patch_validation(self, dataloader, current_iter, tb_logger,
                            save_img=False, rgb2bgr=True, use_image=True,Crop_size=128):
        ...
        # Overlap_size = 64
        # Crop_size = 196
        Overlap_size = Crop_size//2
        for idx, val_data in enumerate(dataloader):
            

            input_lq = val_data['LQ']
            input_ev = val_data['Event']
            input_gt = val_data['GT']
            B, C, H, W = input_lq.shape
            split_lq, starts_lq = splitimage(input_lq, crop_size=Crop_size, overlap_size=Overlap_size)
            input_ev, starts_ev = splitimage(input_ev, crop_size=Crop_size, overlap_size=Overlap_size)
            # input_gt, starts_gt = splitimage(input_gt, crop_size=128, overlap_size=64)

            for i, datai in enumerate(split_lq):
                self.feed_data({"LQ": datai, "Event":input_ev[i],'seq':val_data['seq']})
                if self.opt['val'].get('grids') is not None:
                    self.grids()
                    self.grids_voxel()
                self.test()
                ...
                split_lq[i] = self.get_current_visuals()["result"]
            img_name = self.seq_name
            visuals = mergeimage(
                    split_lq,
                    starts_lq,
            ...
            sr_img = tensor2img(visuals, rgb2bgr=rgb2bgr)
            if 'GT' in val_data:
                gt_img = tensor2img([val_data['GT']], rgb2bgr=rgb2bgr)
                # del self.gt
    ...
    def origin_nondist_validation(self, dataloader, current_iter, tb_logger,
                           save_img, rgb2bgr, use_image):
        dataset_name = self.opt.get('name')
