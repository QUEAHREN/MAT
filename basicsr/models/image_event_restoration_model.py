...
from basicsr.utils import get_root_logger, imwrite, tensor2img, splitimage, mergeimage
...
class ImageEventRestorationModel(BaseModel):
        ...
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                ...
                if k.startswith('module.offsets') or k.startswith('module.dcns'):
                    ...
                    optim_params_highlr.append(v) 
                else:
                    optim_params.append(v)
            else:
                logger = get_root_logger()
                logger.warning(f'Params {k} will not be optimized.')
        # print(optim_params_highlr)
        ratio = 0.1
        ratioh = 1
        optim_type = train_opt['optim_g'].pop('type')
        if optim_type == 'Adam':
            self.optimizer_g = torch.optim.Adam([{'params': optim_params}, {'params': optim_params_lowlr, 'lr': train_opt['optim_g']['lr'] * ratio}, {'params': optim_params_highlr, 'lr': train_opt['optim_g']['lr'] * ratioh}],
                                                **train_opt['optim_g'])
        elif optim_type == 'AdamW':
            self.optimizer_g = torch.optim.AdamW([{'params': optim_params}, {'params': optim_params_lowlr, 'lr': train_opt['optim_g']['lr'] * ratio}, {'params': optim_params_highlr, 'lr': train_opt['optim_g']['lr'] * ratioh}], 
                                                **train_opt['optim_g'])
        elif optim_type == 'SGD':
            self.optimizer_g = torch.optim.SGD(
                                    [
                                        ...
                                        {'params': optim_params_highlr, 'lr': train_opt['optim_g']['lr'] * ratioh}
                                    ],
                                    **{k: v for k, v in train_opt['optim_g'].items() if k == 'lr'}
                                )
        else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['datasets']['train'].get('use_mask'):
            ...
            preds = self.net_g(x = torch.cat([self.lq, self.voxel], 1))
        ...
        #     if name.endswith("in_compress3.0.weight"):
        #         print(f"{name} - Gradient: {param.grad}")
        #     if name.endswith("alpha"):
        #         print(f"{name} - Gradient: {param.grad}")
            # if name.endswith("to_controls.weight"):
            #     print(f"{name} - Gradient: {param.grad}")
            # if name.endswith("conv_1.weight"):
            #     print(f"{name} - Gradient: {param.grad}")
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if self.opt['network_g'].get('use_fsas'):
                    h_n = (32 - h % 32) % 32
                    w_n = (32 - w % 32) % 32
                    lq = F.pad(self.lq[i:j], (0, w_n, 0, h_n), mode='reflect')
                    ev = F.pad(self.voxel[i:j], (0, w_n, 0, h_n), mode='reflect')
                    pred = self.net_g(x = torch.cat([lq, ev],1))
                    pred = pred[:, :, :h, :w]
                
                else:
                    # pred = self.net_g(x = self.lq[i:j, :, :, :], event = self.voxel[i:j, :, :, :])  # mini batch all in 
                    pred = self.net_g(x = torch.cat([self.lq[i:j, :, :, :], self.voxel[i:j, :, :, :]],1))
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                if not self.opt['is_train'] or self.opt['save_as_test']:
